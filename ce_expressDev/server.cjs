...
// This check MUST happen before we 'require' external modules like express
try {
    require.resolve('express');
} catch (e) {
    console.log("\x1b[33m[SYSTEM] Dependencies missing. Initializing first breath...\x1b[0m");
    // This command creates the node_modules folder for the user automatically
    try {
        execSync('npm install express multer cors node-fetch@2', { stdio: 'inherit' });
        console.log("\x1b[32m[SYSTEM] Installation complete. Resuming...\x1b[0m");
    } catch (installError) {
        console.error("\x1b[31m[ERROR] Failed to install dependencies. Please ensure Node.js is installed.\x1b[0m");
        process.exit(1);
    }
}

// --- 3. EXTERNAL ENGINES (Now safe to load) ---
...
// --- 8. TRUST ENGINE (Unified Intent & Persistence) ---
...
// --- 9. ROUTES (Sovereign Interaction) ---
