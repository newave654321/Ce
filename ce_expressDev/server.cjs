...
try {
    ...
    console.log("[SYSTEM] Muscle (Hyperswarm) missing. Installing dependencies...");
    execSync('npm install hyperswarm node-fetch@2 express multer cors', { stdio: 'inherit' });
...
const topic = crypto.createHash('sha256').update('CE-SOVEREIGN-MESH-2026').digest();

let neighbors = new Map();
const messageInboxes = new Map(); // Moved up for safe access
let myHeartbeat = 0; 
let baseRate = 5000; 
...
try {
    ...
    if (fs.existsSync(identityPath)) {
        ...
        fs.writeFileSync(identityPath, JSON.stringify({ nodeId: myFullIdentity, isLocked: false }, null, 2));
    }
} catch (err) { console.error("Identity access restricted."); }

// --- 6. TRUST ENGINE (Unified Intent & Persistence) ---
const karmAMap = new Map();
const DECAY_INTERVAL = 60000; 
const DECAY_RATE = 0.005;
const GAINS = { LISTEN: 0.025, OTHER: 0.005, UTILITY_WORK: 0.010, KNOCK: 0.100 };
...
function applyDecay() {
    ...
    karmAMap.forEach((data, nodeId) => {
        ...
        karmAMap.set(nodeId, { ...data, value: fadedValue });
    });
}
setInterval(applyDecay, DECAY_INTERVAL);

// Restoring the missing logic for the Trust Firewall
function calculateKarmA(nodeId, type, observedInterval = null) {
    let entry = karmAMap.get(nodeId) || { value: 0.1, lastSeen: Date.now() };
    
    // THE TIME FIREWALL: Penalize sub-human speeds (bots)
    if (observedInterval !== null && observedInterval < MIN_HUMAN_INTERVAL) {
        let penalized = Math.max(entry.value - 0.05, 0.001);
        karmAMap.set(nodeId, { value: penalized, lastSeen: Date.now() });
        return penalized;
    }
    
    let gain = GAINS[type] || GAINS.OTHER;
    let updated = Math.min(entry.value + gain, 1.0);
    karmAMap.set(nodeId, { value: updated, lastSeen: Date.now() });
    return updated;
}

// --- 7. ROUTES (Sovereign Interaction) ---

// This allows the UI to show the "Prospectors" (Nearby Nodes)
app.get('/api/sidebar-data', (req, res) => {
    const prospectors = Array.from(neighbors.values()).filter(n => Date.now() - n.lastSeen < 3600000).length;
    res.json({ prospectors });
});

// This allows the UI to display incoming Agreement requests
...
// This allows the UI to fetch and then clear the local message inbox
app.get('/api/get-messages/:hash', (req, res) => {
    const { hash } = req.params;
    const inbox = messageInboxes.get(hash) || [];
    messageInboxes.set(hash, []); // Clear after read to respect resource limits
...
function breathe() {
    myHeartbeat++; 
    const pulse = Buffer.from(JSON.stringify({ name: myPublicName, type: 'PULSE', heartbeat: myHeartbeat, timestamp: Date.now() }));
...
scanner.on('message', (msg, rinfo) => {
    try {
        ...
        if (data.type === 'PULSE') {
            ...
            const interval = lastData ? (Date.now() - lastData.lastSeen) : null;
            calculateKarmA(data.name, 'LISTEN', interval);
            neighbors.set(data.name, { id: data.name, ip: rinfo.address, lastSeen: Date.now(), heartbeat: data.heartbeat });
        } 
            ...
            if (JSON.stringify(myData).toLowerCase().includes(data.subject.toLowerCase())) {
                const logPath = path.join(__dirname, 'agreements_pending.json');
                let pending = fs.existsSync(logPath) ? JSON.parse(fs.readFileSync(logPath, 'utf8')) : [];
                pending.push({ id: Date.now(), from: data.from, action: data.action, subject: data.subject, timestamp: new Date().toISOString() });
...
app.listen(3000, '0.0.0.0', () => { console.log(`\x1b[42m CE BRAIN ONLINE \x1b[0m`); });